...
const Community = () => {
  ...
  const handleSubmite = (e) => {
    ...
    Axios.post("https://api.cloudinary.com/v1_1/andela-rwanda-kigali/image/upload", formData).then((response) => {
      ...
      const Credentials = { name, diocese, member, fbLink, twLink, image: protifolioImage }
      setLoading(true)
      ...
      <div className={`h-screen w-screen bg-gray-600 bg-opacity-30 backdrop-blur-sm fixed flex items-center justify-center px-4 ${deleteCohortModel === true ? 'block' : 'hidden'}`}>
        <div className="bg-white w-full sm:w-3/4 md:w-1/2  xl:w-4/12 rounded-lg p-4 pb-8">
          ...
          <div className="card-body">
            <form className=" px-8" >
              <div>
                <h2 className='text-base m-4'>Do you really want to remove <span className='italic text-black'>{RowData.name}</span> from the list</h2>
              </div>
              <div className="w-full flex justify-between">
        ...
        <div className="px-3 md:px-6 lg:px-28">
          <div className="bg-white shadow-lg px-5 py-8 rounded-md w-full  lg:w-full ">
            <div className=" flex items-center justify-between pb-6">
              <div>
                <h2 className="font-sans text-xl text-gray-700 font-semibold px-1 hover:underline">Communities</h2>
              </div>
            </div>
            <div>
                    ...
                    <tbody>
                      {Data.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">{item.member}</p>
                          </td>
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p className="text-gray-900 whitespace-no-wrap">
                              {item.diocese}
                          ...
                          <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div className='flex'>
                              ...
                              <a href={twLink} target="_blank" rel="noreferrer"><div className="cursor-pointer mr-4 text-blue-800">
                                <FaTwitter />
                              </div></a>
                            </div>
                          </td>
